        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="fine in fines" :key="fine.index">
              <td scope="row">
                <p class="text-center">Amenda: {{ fine.fine_id }}</p>
              </td>
            ...
            <tr v-for="notice in notices" :key="notice.index">
              ...
              <td scope="row">
                <p class="text-center">{{ notice.damage_cost }} lei</p>
              </td>
            </tr>
          </tbody>
        </table>
        <table class="table table-striped">
          <thead class="text-center">
            <tr class="text-center">
              <th scope="col">
                <p class="text-center">Suma totala a costurilor</p>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td scope="row">
                <p class="text-center">{{ totalSumCount() }}</p>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  computed: {
    filterEmployees() {
      return this.notices
        .filter(
          (notice) =>
            notice.notice_id.toString().indexOf(this.filterId.toLowerCase()) >
            -1
        )
        .filter(
          (notice) =>
