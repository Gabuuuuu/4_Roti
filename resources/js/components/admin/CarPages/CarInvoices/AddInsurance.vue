                ...
                <div class="card-body p-5 text-center">
                  <div class="mb-md-6 mt-md-4 pb-5">
                    <h2 class="fw-bold mb-4">
                      Inregistreaza o asigurare pentru o masina
                    </h2>
                    ...
                    >
                      <div class="form-outline form-white mb-4">
                        <p>
                          Alege masina pentru care a fost achizitionata
                          asigurarea
                      ...
                      <div class="form-outline form-white mb-4">
                        <p>
                          Alege firma de asigurari alaturi de care a fost facuta
                          asigurarea
                          ...
                          >
                            <option
                              ...
                              :key="insurance_company.insurance_company_id"
                            >
                              {{
                                insurance_company.denumire_companie_asigurari
                              }}
                            </option>
                          </select>
                        </p>
                        ...
                        <p>
                          ...
                          >
                            <option
                              ...
                              :key="insurance_type.insurance_type_id"
                            >
                              {{ insurance_type.tip_asigurare }}
                            </option>
                          </select>
                        </p>
                      </div>
...
export default {
  ...
  methods: {
    addinsurance() {
      ...
      const companie_asigurari = this.insurance_companies.filter(
        (item) =>
          item.denumire_companie_asigurari === this.form.insurance_company
      )[0].insurance_company_id;
      const tip_asigurare = this.insurance_types.filter(
        (item) => item.tip_asigurare === this.form.insurance_type
      )[0].insurance_type_id;
      const data = new FormData();
      data.append("car_id", car_id);
      data.append("insurance_company_id", companie_asigurari);
      data.append("insurance_type_id", tip_asigurare);
      data.append("Data_emiterii", this.form.date);
      data.append("Pret_asigurare", this.form.price);

      axios.post("/api/insurances", data).then(() => {
    ...
    async loadData() {
      ...
      axios.all([loadCompany, loadType]).then(
        axios.spread((...responses) => {
          ...
          this.form.insurance_company =
            this.insurance_companies[0].denumire_companie_asigurari;
          this.form.insurance_type = this.insurance_types[0].tip_asigurare;
        })
      );
    },
  },
